...
#list(
  spacing: 5pt,
  item("Миграция легаси-движка")[Перевёл живой Unity-клиент с 2017 на Unity 6.3 шестью LTS-этапами, не останавливая продукт. Каждый этап закрыт тегом-чекпоинтом и проверен компиляцией, боем 5v5, standalone-билдом и сборкой ассет-бандлов.],
  ...
  item("Диагностика инцидента на проде")[Нашёл корень амплификации DDoS в слое RUDP по 46 клиентским сессиям и 2 краш-дампам. Убрал O(n)-скан на каждый неизвестный пакет и лог на каждый пакет за флагом failover, закрыл 5 регрессионными тестами.],
  item("Фичи и платформы")[Снизил потребление памяти на 30% и CPU на 20% за счёт allocation-free паттернов. Довёл до релиза 15+ фич, включая батл-пасс, с интеграцией Firebase, AppsFlyer, Pushwoosh, Unity IAP и Xsolla.],
  item("Менторство")[Провёл мид-программиста через испытательный срок и перевёл команду на allocation-free код с задокументированным пайплайном работы с ассетами.],
)
